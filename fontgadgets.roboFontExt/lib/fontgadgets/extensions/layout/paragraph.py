from fontgadgets.extensions.layout.segmenting import get_base_level
import unicodedata2
...
class Paragraph:
    ...
    Attributes:
        baseLevel (int, optional): Paragraph's base bidi level (0=LTR, 1=RTL).
    ...
    def __init__(self, baseLevel=None, width=None, lineHeight=None):
    ...
    def addTextFromFont(self, text, font, features=None, language=None, script=None, variations=None):
        ...
        if self._baseLevel is None:
            self._baseLevel = get_base_level(text)
...
def breakGlyphRunsUsingLineWidth(glyphRuns, lineWidth, direction=0):
    ...
    if direction == 0:
        ...
        remainingGlyphRuns = list(glyphRuns)
        ...
        while remainingGlyphRuns:
            ...
            if addedGlyphRunsWidth + currentGlyphRun.width <= lineWidth:
                ...
                if breakIndex is not None:
                    ...
                    # last glyph is wider than line width
                    return glyphLines
        if currentGlyphRuns:
            if direction == 1:
                currentGlyphRuns = reversed(currentGlyphRuns)
