...
class GlyphGit:
    ...
    def copyContentsFromCommit(
        ...
        """
        Copies selected data from a historical commit of the current glyph to a target glyph.

        This method retrieves the state of the glyph (represented by `self._glyph`)
        at a specific Git commit and copies its attributes to `other_glyph`.
        If `other_glyph` is not provided, the data is copied back into the
        original glyph.
        ...
        Args:
            ...
            base_glyphs (bool, optional): If True, and the glyph at the commit contains
                ...
                `commit_index` in the target font. Defaults to True.
