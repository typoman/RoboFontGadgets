...
class ParagraphTest(unittest.TestCase):
    ...
    def test_long_bidi_phrase_1(self):
        glyphLines = self.shapeTextToGlyphLines("/en/خط/fa", 2200)
        self.assertGlyphLinesTextEqual(glyphLines, ["/en/", "خط/", "fa"])

    def test_long_bidi_phrase_2(self):
        glyphLines = self.shapeTextToGlyphLines("از a تا z؟", 2200)
        self.assertGlyphLinesTextEqual(glyphLines, ['از a', 'تا  z', '؟'])

    def test_long_bidi_phrase_3(self):
        glyphLines = self.shapeTextToGlyphLines("From ا to ی?", 2200)
        self.assertGlyphLinesTextEqual(glyphLines, ['Fro', 'm ا ', 'to ', 'ی?'])
